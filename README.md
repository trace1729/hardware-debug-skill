...
该 skill 面向硬件波形调试场景，结合波形、emitted RTL 与 Scala/Chisel 源码，为 LLM 提供结构化调试入口。
...
- `--vcd`：用于 `inspect-inputs` 与 `build-wave-db` 的波形路径
...
- 检查 `--scala-root`、`--vcd`、`--rtl-root` 是否存在
- 估算波形与源码树规模
- 输出默认 artifact 路径
- 优先打印直接波形查询命令
...
- 当前推荐路径虽然依赖 `wellen`，但 `inspect-inputs` 与备用预处理命令仍沿用 `--vcd` 参数名。
- `build-wave-db` 目前仍是以 VCD 为中心的预处理实现，并未替换为 `wellen` 全量落盘。
